...
void init_regex() {
  ...
  for (i = 0; i < (int)(sizeof(rules) / sizeof(rules[0])); i ++) {
    ...
    if (ret != 0) {
      ...
      do { if (!(0)) { (fflush(stdout), fprintf(stderr, "\33[1;31m" "regex compilation failed: %s\n%s" "\33[0m" "\n", error_msg, rules[i].regex)); extern FILE* log_fp; fflush(log_fp); extern void assert_fail_msg(); assert_fail_msg(); ((void) sizeof ((0) ? 1 : 0), __extension__ ({ if (0) ; else __assert_fail ("0", "src/monitor/sdb/expr.c", 77, __extension__ __PRETTY_FUNCTION__); })); } } while (0);
    }
  }
}
...
static _Bool make_token(char *e) {
  int position = 0;
  nr_token = 0;
  int i;
  while (e[position] != '\0') {
  ...
  for (int i = 0; i < nr_token; i++) {
    if (tokens[i].type == TK_REG) {
      ...
      int val = strtol(tokens[i].str, ((void *)0), 16);
      intToString(val, tokens[i].str);
        ...
        nr_token--;
              ...
              || (tokens[i].type == TK_MUL && i == 0)) {
                ...
                if (tokens[i + 1].type == TK_HEX) {
                  val = strtol(tokens[i + 1].str, ((void *)0), 16);
...
long long expr(char *e, _Bool *success) {
  if (!make_token(e)) {
    *success = 0;
    return 0;
  }
