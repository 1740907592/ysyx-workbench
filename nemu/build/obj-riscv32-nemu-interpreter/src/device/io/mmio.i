...
static void report_mmio_overlap(const char *name1, paddr_t l1, paddr_t r1,
  ...
  do { if (!(0)) { (fflush(stdout), fprintf(stderr, "\33[1;31m" "MMIO region %s@[" "0x%08" "x" ", " "0x%08" "x" "] is overlapped " "with %s@[" "0x%08" "x" ", " "0x%08" "x" "]" "\33[0m" "\n", name1, l1, r1, name2, l2, r2)); extern FILE* log_fp; fflush(log_fp); extern void assert_fail_msg(); assert_fail_msg(); ((void) sizeof ((0) ? 1 : 0), __extension__ ({ if (0) ; else __assert_fail ("0", "src/device/io/mmio.c", 31, __extension__ __PRETTY_FUNCTION__); })); } } while (0);
}
void add_mmio_map(const char *name, paddr_t addr, void *space, uint32_t len, io_callback_t callback) {
  ((void) sizeof ((nr_map < 16) ? 1 : 0), __extension__ ({ if (nr_map < 16) ; else __assert_fail ("nr_map < NR_MAP", "src/device/io/mmio.c", 37, __extension__ __PRETTY_FUNCTION__); }));
  ...
  do { printf("\33[1;34m" "[%s:%d %s] " "Add mmio map '%s' at [" "0x%08" "x" ", " "0x%08" "x" "]" "\33[0m" "\n", "src/device/io/mmio.c", 50, __func__, maps[nr_map].name, maps[nr_map].low, maps[nr_map].high); do { extern FILE* log_fp; extern _Bool log_enable(); if (log_enable() && log_fp != ((void *)0)) { fprintf(log_fp, "\33[1;34m" "[%s:%d %s] " "Add mmio map '%s' at [" "0x%08" "x" ", " "0x%08" "x" "]" "\33[0m" "\n", "src/device/io/mmio.c", 50, __func__, maps[nr_map].name, maps[nr_map].low, maps[nr_map].high); fflush(log_fp); } } while (0); } while (0);
  nr_map ++;
}
