...
static inline _Bool map_inside(IOMap *map, paddr_t addr) {
...
static uint8_t *io_space = ((void *)0);
static uint8_t *p_space = ((void *)0);
uint8_t* new_space(int size) {
  uint8_t *p = p_space;
  size = (size + ((1ul << 12) - 1)) & ~((1ul << 12) - 1);
  p_space += size;
  ((void) sizeof ((p_space - io_space < (32 * 1024 * 1024)) ? 1 : 0), __extension__ ({ if (p_space - io_space < (32 * 1024 * 1024)) ; else __assert_fail ("p_space - io_space < IO_SPACE_MAX", "src/device/io/map.c", 31, __extension__ __PRETTY_FUNCTION__); }));
  return p;
}
static void check_bound(IOMap *map, paddr_t addr) {
  if (map == ((void *)0)) {
    do { if (!(map != ((void *)0))) { (fflush(stdout), fprintf(stderr, "\33[1;31m" "address (" "0x%08" "x" ") is out of bound at pc = " "0x%08" "x" "\33[0m" "\n", addr, cpu.pc)); extern FILE* log_fp; fflush(log_fp); extern void assert_fail_msg(); assert_fail_msg(); ((void) sizeof ((map != ((void *)0)) ? 1 : 0), __extension__ ({ if (map != ((void *)0)) ; else __assert_fail ("map != ((void *)0)", "src/device/io/map.c", 37, __extension__ __PRETTY_FUNCTION__); })); } } while (0);
  } else {
    do { if (!(addr <= map->high && addr >= map->low)) { (fflush(stdout), fprintf(stderr, "\33[1;31m" "address (" "0x%08" "x" ") is out of bound {%s} [" "0x%08" "x" ", " "0x%08" "x" "] at pc = " "0x%08" "x" "\33[0m" "\n", addr, map->name, map->low, map->high, cpu.pc)); extern FILE* log_fp; fflush(log_fp); extern void assert_fail_msg(); assert_fail_msg(); ((void) sizeof ((addr <= map->high && addr >= map->low) ? 1 : 0), __extension__ ({ if (addr <= map->high && addr >= map->low) ; else __assert_fail ("addr <= map->high && addr >= map->low", "src/device/io/map.c", 39, __extension__ __PRETTY_FUNCTION__); })); } } while (0);
  }
}
static void invoke_callback(io_callback_t c, paddr_t offset, int len, _Bool is_write) {
  if (c != ((void *)0)) { c(offset, len, is_write); }
}
void init_map() {
  io_space = malloc((32 * 1024 * 1024));
  ((void) sizeof ((io_space) ? 1 : 0), __extension__ ({ if (io_space) ; else __assert_fail ("io_space", "src/device/io/map.c", 51, __extension__ __PRETTY_FUNCTION__); }));
  p_space = io_space;
}
word_t map_read(paddr_t addr, int len, IOMap *map) {
  ((void) sizeof ((len >= 1 && len <= 8) ? 1 : 0), __extension__ ({ if (len >= 1 && len <= 8) ; else __assert_fail ("len >= 1 && len <= 8", "src/device/io/map.c", 56, __extension__ __PRETTY_FUNCTION__); }));
...
void map_write(paddr_t addr, int len, word_t data, IOMap *map) {
  ((void) sizeof ((len >= 1 && len <= 8) ? 1 : 0), __extension__ ({ if (len >= 1 && len <= 8) ; else __assert_fail ("len >= 1 && len <= 8", "src/device/io/map.c", 65, __extension__ __PRETTY_FUNCTION__); }));
  check_bound(map, addr);
  paddr_t offset = addr - map->low;
